...
static std::shared_ptr<dae::GameObject> CreateLevel(dae::Scene& scene, const std::string& levelPath)
{
	auto pLevelObject = std::make_shared<dae::GameObject>();
	scene.Add(pLevelObject);
	pLevelObject->SetTag("Level");

	auto font = dae::ResourceManager::GetInstance().LoadFont("Lingua.otf", 13);

	const auto skipObj = std::make_shared<dae::GameObject>();
	const auto skipText = std::make_shared<dae::TextComponent>(skipObj.get(), "Skip Level (N/Button Y)", font, SDL_Color{ 255, 255, 255, 255 });
	skipObj->SetRelativePos({ 250, 458 });
	skipObj->AddComponent(skipText);
	pLevelObject->AddChild(skipObj);

	const auto muteObj = std::make_shared<dae::GameObject>();
	const auto muteText = std::make_shared<dae::TextComponent>(muteObj.get(), "Mute (M/Button Back)", font, SDL_Color{ 255, 255, 255, 255 });
	...
	int curRow{};
	int curCol{};
	const auto map = dae::ResourceManager::GetInstance().ParseCsv(levelPath);
	...
	for (size_t i{}; i < map.size(); ++i)
	{
		...
		case 2:
			...
			if (curRow % 2 == 1 && curCol % 2 == 1 &&
				curCol < amountCols - 6)
			{
				teleportPlaces.emplace_back(static_cast<int>(i));
			}
			break;
		case 3:
			pTexture->SetTexture("Resources/Level/path.png");
